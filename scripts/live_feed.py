    ...
    with connect() as conn, conn.cursor() as cur:
        for table in company_tables(cur):
            ...
            if DRY_RUN:
                ...
                cur.execute(stmt)
                print(f"  {table}: +1 row for {d}")


def main():
    print(f"live_feed → {CATALOG}.{SCHEMA} ({'DRY RUN' if DRY_RUN else 'writing'}), "
